...
interface KaiStreamPageProps {
  searchParams: { task?: string };
}

export default function KaiStreamPage({ searchParams }: KaiStreamPageProps) {
  const taskKey = searchParams.task || 'JKILL-263';
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        const response = await fetch(`/api/kai-stream-history?task=${taskKey}`);
          ...
          {events.length === 0 ? (
            <div className="text-white/40 text-center py-12">
              ...
              <span className="text-xs">
                Execute ./kai-delegate-simple.sh {taskKey}
              </span>
            </div>
          ) : (
